...
以下の文字列の配列として定義します。空配列 `[]` の場合は受動発動などの特殊カードとなります。スーパーミラーのような例外はシステム側で個別対応します。

対応する定数は `godfield_core/src/card_registry.cpp` の変換表（`TIMING_*`）が持ちます。フェイズ自体の説明は [`phase_specifications.md`](./phase_specifications.md) を参照してください。
