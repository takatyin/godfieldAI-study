...
「夢グループ」の仕様が正しく機能している限り、通常プレイ中に「見た目ベースで合法だったアクションが、確定した後に非合法になる」ということは発生しません。しかし、エンジン側のバグや定義ミスによるクラッシュを防ぎ、堅牢性を保つ防衛策（セーフガード）として、以下のロールバック処理が `step_game`（[game_logic.cpp](file:///c:/Users/onaok/Desktop/git/GodFieldAI/godfield_core/src/game_logic.cpp)）に実装されています。
