...
print(f"After select0 (ID_SELL): phase={runner.state.current_phase}, actor={runner.state.current_actor_id}, staged={runner.state.get_staged_cards(0,0)}, num={runner.state.get_num_staged_cards(0)}")
runner.step(action=ActionType.ACTION_TARGET_SELF)
print(f"After target self: phase={runner.state.current_phase}, actor={runner.state.current_actor_id}, attacker={runner.state.attacker_id}, defender={runner.state.defender_id}")
runner.step(action=ActionType.ACTION_SELECT_HAND_1)
print(f"After select1 (pot): phase={runner.state.current_phase}, actor={runner.state.current_actor_id}, staged1={runner.state.get_staged_cards(0,1)}, num={runner.state.get_num_staged_cards(0)}")
