...
#   v4 … 属性表が中心化されておらず、追加した経路が既存の表現を潰していた。
#         ゲートを足したぶん重みが増えたので、もう読めない。
#         この版で学習したモデルは対 strategic 21.0%（v3 は 44.9%）で、
#         approx_kl が 0.002 に張り付いて学習が止まっていた。使わないこと。
#
# 詳細は docs/rl_history.md を参照。
POOL_DIR="${POOL_DIR:-models/league_v5}"
